...
void main()
{
	vec4 color1 = texture2D( u_diffuseSampler1, v_texC * u_properties.TileRepeatFactor.x );
	vec4 color2 = texture2D( u_diffuseSampler2, v_texC * u_properties.TileRepeatFactor.y );
	vec4 blend = texture2D( u_blendSampler, v_texC * u_properties.TileRepeatFactor.z );
